...
async function main() {
    ...
    const whaleSponsorAddress = addresses.WHALES.DAI[0];
  ...
  console.log('Whale Dai balance:', ethers.formatUnits(whaleDaiBalance, 18)); 
    ...
    const abbot = addresses.HARDHAT_ACCOUNTS[0];

    const abbotSigner = await ethers.getSigner(abbot.account);
    ...
    const costelloSigner = await ethers.getSigner(costello.account);
